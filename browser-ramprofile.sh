...
set_browser() {
  ...
  case "$browser_prefix" in
    "chromium")
      ...
      browser_config_store="${datastore}/${browser_prefix}_config.tar.${compression_method}"
      browser_cache_store="${datastore}/${browser_prefix}_cache.tar.${compression_method}"
      ;;
    "firefox")
...
sync_profile() {
  ...
  tar -c ./config | lbzip2 -zc >"${browser_config_store}.Unconfrimed"
  let "retVal=$?"
  ...
  tar -c ./cache | lbzip2 -zc >"${browser_cache_store}.Unconfrimed"
  let "retVal=$?"
